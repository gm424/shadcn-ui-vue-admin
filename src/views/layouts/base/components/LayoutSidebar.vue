<template>
  <aside
    :class="
      cn(
        `fixed left-0 rounded-lg right-0 top-0 z-50 w-full border-r-2 border-r-muted transition-[width] md:bottom-0 md:right-auto md:h-svh ${
  ...
  >
    <div :class="cn('relative flex h-full w-full flex-col')">
      ...
      <div
        :class="
          cn(
            `flex items-center h-[var(--header-height)] flex-none gap-4 bg-background p-4 pt-3.5 md:px-8 border-b ${
              !isCollapsed ? 'gap-2' : ''
            }`
          )
        "
      ...
      <!-- Navigation links -->
      <div
        :class="cn(`h-full flex-1 px-6 overflow-auto ${isCollapsed ? 'max-h-screen' : 'max-h-0 py-0 md:max-h-screen'}`)"
      >
        <div class="bg-background py-2 h-full duration-500 data-[collapsed=true]:py-2 md:border-none space-y-1">
          <div v-for="(item, index) in navigators" :key="index" class="text-center">
