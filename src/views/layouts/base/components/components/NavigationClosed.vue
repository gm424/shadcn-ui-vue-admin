<template>
  <div
    v-if="navigator?.position === NavigationPosition.LEFT_TOP || navigator?.position === NavigationPosition.LEFT_BOTTOM"
  >
    <Collapsible
      v-model="navigator.opened"
      v-if="navigator?.children && navigator.children.length > 0"
      class="cursor-pointer"
      @click="navigator.opened = !navigator.opened"
    >
      <CollapsibleTrigger as-child>
        <Button
          as="a"
          size="sm"
          class="flex items-center justify-between text-wrap rounded-none h-10 w-full border-l-slate-500 px-2"
        ...
        >
          <div class="flex items-center">
            ...
            <div
              v-if="navigator?.label"
              class="ml-2 rounded-lg bg-primary px-2 mt-0.5 text-[0.625rem] text-primary-foreground"
            >
              {{ navigator?.label }}
            </div>
          </div>
      ...
      <CollapsibleContent class="space-y-2">
        <NavigationRouterLink
          ...
          :class="`my-1 ml-8 ${
            item?.position === NavigationPosition.LEFT_BOTTOM ? 'absolute bottom-0 left-0 right-0' : ''
          }`"
        >
          <Button
            as="a"
            size="sm"
            class="justify-start text-wrap rounded-none h-10 w-full border-l-slate-500 px-2"
    ...
    <NavigationRouterLink
      v-else
      :external="navigator?.external"
      :link="navigator?.href as string"
